...
| FR-02 | Nhập kho tự động | 🟢 Core hoàn thành | Bảng `INBOUND_RECEIPT`, `INBOUND_RECEIPT_LINE`; package `PKG_INBOUND` (`create_receipt`/`add_line`/`receive_line`/`putaway_line`/`complete_receipt`) — đã test end-to-end (nhận hàng vào dock RECEIVING → putaway sang bin → complete, đều post đúng qua `PKG_INVENTORY`). 1 trang APEX (Inbound Receipts, page 60) gộp header+dòng theo kiểu Interactive Grid Master-Detail (chọn 1 phiếu nhập ở lưới trên → lưới dòng bên dưới tự lọc theo phiếu đó, dòng mới tự gán đúng phiếu), Supplier/Warehouse/Item/Lot/Location là dropdown thật ngay trong lưới; nằm trong nhóm menu Inventory. **Còn thiếu**: trang thao tác "Receive & Putaway" dạng scan (khác với CRUD hiện có, cần cho UX vận hành thực tế của nhân viên kho) |
