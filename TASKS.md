...
| --- | --- | --- | --- |
| FR-01 | CSDL tập trung tồn kho (item/vị trí/lô) | 🟢 Hoàn thành (schema+API+UI) | Bảng `SUPPLIER/WAREHOUSE/LOCATION/ITEM/LOT/UOM/INVENTORY_BALANCE/INVENTORY_TRANSACTION` + view `V_INVENTORY_CURRENT` trong DB thật, có seed data. Package `PKG_ITEM/PKG_LOCATION/PKG_LOT/PKG_INVENTORY` đã compile sạch, đã test end-to-end (create/update, QC transition, post transaction, allocate/release, chặn âm kho — đều đúng). Trang Items/Locations đã chuyển sang mô hình **Report (chỉ đọc) + Form (modal)**: cột many2one (UOM, Warehouse) là dropdown thật lọc theo `is_active='Y'`, cột boolean là select Yes/No mặc định Yes (xem `CLAUDE.md` mục "Mẫu Report + Form cho many2one"). Thêm 2 trang mới Warehouses/Suppliers (trước đây chưa có UI quản lý). 8 trang APEX master data (Items, Locations, Lots, UOM, Warehouses, Suppliers, Inventory Balance, Inventory Transactions) đã import vào app 105. **Còn thiếu duy nhất**: baseline KPI thủ công (cần số liệu thật từ người dùng/business, không thể tự tạo) |
| FR-02 | Nhập kho tự động | 🟢 Core hoàn thành | Bảng `INBOUND_RECEIPT`, `INBOUND_RECEIPT_LINE`; package `PKG_INBOUND` (`create_receipt`/`add_line`/`receive_line`/`putaway_line`/`complete_receipt`) — đã test end-to-end (nhận hàng vào dock RECEIVING → putaway sang bin → complete, đều post đúng qua `PKG_INVENTORY`). 2 trang APEX (Inbound Receipts, Inbound Receipt Lines), đã chuyển sang Report+Form với Supplier/Warehouse/Item/Lot/Location là dropdown thật. **Còn thiếu**: trang thao tác "Receive & Putaway" dạng scan (khác với form CRUD hiện có, cần cho UX vận hành thực tế của nhân viên kho) |
