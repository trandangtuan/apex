...
1. Resolve `db_connection_name` through saved-connection discovery or manual fallback. Require an exact workspace name only for new-app materialization; for existing apps, resolve workspace id only if the active runtime reports ambiguity.
...
5. Normalize every `.apx` source file from CRLF or CR to LF and verify the result before live `apex validate` or `apex import`. Block the runtime lane with `APEXLANG_LF_LINE_ENDINGS_REQUIRED_001` if normalization cannot be completed.
6. Treat the internal validate-only roundtrip as the `APEXlang source import` lane. It validates the staged source tree and is not interchangeable with any separate compiled SQL export import path.
7. Treat preflight as a capability, live-metadata, and target-identity gate. When it successfully resolves runtime capability and live metadata, the run may continue even if the preflight stage budget was exceeded.
8. Treat live `apex validate` as the first mandatory blocking runtime action in the happy path. For APEX/ORA runtime error text with a live DB connection, if runtime was to actually be tested, query `APEX_DEBUG_MESSAGES` through `references/domains/debugging/apex-debug-messages.md` to resolve the `PAGE_VIEW_ID` execution log before assigning ownership. Route the finding through the same debug/fix/revalidate loop before import is allowed.
9. For an approved import continuation, run `node tools/apexctl.mjs runtime roundtrip --app-path <final_app_path> --db-connection-name <db_connection_name> --import-intent validate-and-import [--execution-mode auto|build-root|path]` and execute live import immediately after successful live validate. In user-facing responses, label this path as `Check and import APEXlang code`.
10. Browser/runtime verification is not a pre-import gate. Run it only after `import_status = pass`, and only when the caller explicitly requests it with `--require-runtime-verification`.
11. Treat post-import runtime verification findings as diagnostics. Record them in the transcript/report and `runtime_verification_*` fields, but do not rewrite a successful import to fail.
12. The runtime executor runs in fixed phases with immutable handoff facts: `preflight`, `local_validate`, `target_resolve`, `live_validate`, and `import`. Each phase records duration, budget, status, failure class, and next safe action in the stage report.
13. `local_validate` remains an advisory/helper phase. It records whether the local first-pass check ran or was skipped by roundtrip policy, but it is no longer the first mandatory blocker for the live roundtrip lane. LF normalization remains mandatory and independent of this advisory policy.
14. The default roundtrip validates/imports the resolved app path in place. Use transient runtime directories only for logs, stage reports, and temporary SQL scripts. Full app-copy staging and sync-back are reserved for workflows that intentionally mutate artifacts before publish.
15. For existing-app `validate-and-import` runs, resolve the live canonical numeric application id before import, preserve it as session authority, and reconcile staged deployment metadata to that canonical id before import-authorized runtime work continues.
16. If the local first-pass check already passed and a sandboxed build-root attempt fails before real `apex validate` / `apex import` output because of filesystem/setup errors such as `EPERM`, `ENOENT`, or build-root `workdir/*` write failures, stop sandbox retries and continue with the real live build-root roundtrip.
17. If the active runtime session explicitly reports multiple-workspace ambiguity, automatically resolve the workspace id for the active `db_connection_name` and restart the same roundtrip immediately with a run-scoped explicit `-workspaceid`.
18. Optionally run `apex export -applicationid <application_id> -exptype APEXLANG -split -dir <absolute_export_dir>` only when the selected runtime path confirms export support.
19. When export backups are produced, create the output directory lazily and write them under `APEXLANG_OUTPUT_ROOT/apex-exports/<app>/`, never under `applications/<app>/`.
...
- The runtime roundtrip assumes the connection gate already resolved `db_mode` and required `db_connection_name`; it is not responsible for prompting the user through the staged offline/live DB flow.
