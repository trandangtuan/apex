...
4. Resolve prerequisite metadata and saved connections first for APEX artifact work: inspect offline schema dictionaries, scan saved SQLcl connections, auto-bind one deterministic alias, and ask for manual `db_connection_name` only when discovery remains unresolved. Require a workspace name for new-app materialization or resolve a workspace id after runtime-reported ambiguity.
